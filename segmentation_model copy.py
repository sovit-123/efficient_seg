...
class EffSegModel(nn.Module):
    def __init__(self, num_classes=1, pretrained=True, aux=False):
        ...
        self.aux_out = FCNHead(320, num_classes) if aux else None
    ...
    def forward(self, x):
        ...
        if self.aux_out is not None:
            aux = self.aux_out(x)
            aux_out = x = F.interpolate(aux, size, mode='bilinear', align_corners=True)
        else:
            aux_out = None
        x = self.head(x)
...
if __name__ == '__main__':
    model = EffSegModel(num_classes=2, aux=True)
    print(model)
